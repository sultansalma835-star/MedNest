...
export default function Inventory() {
  ...
  useEffect(() => {
    ...
    // 🔥 جلب كل inventory + stats
    fetch("http://127.0.0.1:8000/api/pharmacist/inventory", config)
      .then((res) => {
    ...
    // 🔥 جلب expiring API (المهم اللي طلبته)
    fetch("http://127.0.0.1:8000/api/pharmacist/inventory/expiring", config)
      .then((res) => {
        if (!res.ok) throw new Error("Expiring API failed");
        return res.json();
      })
      .then((data) => {
        setExpiringCount(data.count || 0);
        setExpiringList(data.data || []);
      })
      .catch((err) => {
        console.log(err);
        setError("Failed to load expiring medicines");
      });
  }, [token]);
      ...
      <div className="inventory-container">
        ...
        <div className="stats">
          ...
          <div className="stat-box expiring">
            ...
            <div className="stat-number">
              <span className="num-badge">{expiringCount}</span>
            </div>
          </div>
        </div>
          ...
          <tbody>
            {medicines.length === 0 ? (
              ...
              medicines.map((med) => {
                ...
                if (med.quantity === 0) statusText = "Out of Stock";
                else if (med.is_low_stock) statusText = "Low Stock";
                else if (med.is_expiring_soon) statusText = "Expiring";
        ...
        {/* 🔥 TABLE expiring API */}
        <h3 style={{ marginTop: "40px" }}>Expiring Soon Medicines</h3>
